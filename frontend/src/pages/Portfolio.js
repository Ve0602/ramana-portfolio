...
export default function Portfolio() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/projects`).then(r => setProjects(r.data)).catch(() => setProjects([]));
  }, []);
  ...
  return (
    <div>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', padding: '100px 60px 60px', position: 'relative', overflow: 'hidden', background: 'var(--bg)' }}>
        ...
        <div style={{ maxWidth: 700, position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ fontFamily: 'Syne,sans-serif', fontSize: 'clamp(42px,6vw,76px)', fontWeight: 800, lineHeight: 1.05, marginBottom: 20 }}>
            {(d.hero?.name || 'Ramana Vemunoori').split(' ')[0]}<br />
            <span style={{ color: 'var(--accent)' }}>{(d.hero?.name || 'Ramana Vemunoori').split(' ')[1]}</span>
          </h1>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 56 }}>
            ...
            <Link to="/resume" style={btnSecondary}>📄 Resume</Link>
            <Link to="/freelance" style={btnSecondary}>💼 Hire Me</Link>
      ...
      <div className="psec" id="experience" style={{ background: 'var(--surface)' }}>
        ...
        <div style={{ position: 'relative', paddingLeft: 28, borderLeft: '2px solid var(--accent)' }}>
            ...
            <div key={i} style={{ position: 'relative', marginBottom: 44, paddingLeft: 28 }}>
              ...
              <div style={card}>
                ...
                <ul style={{ paddingLeft: 18 }}>
                  {exp.points.map((p, j) => <li key={j} style={{ color: '#9ca3af', fontSize: 14, marginBottom: 7, lineHeight: 1.6 }}>{p}</li>)}
                </ul>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 20 }}>
          {(d.skills || []).map(s => (
            <div key={s.name} style={{ ...card, transition: 'all 0.2s' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(0,212,255,0.25)'; e.currentTarget.style.transform = 'translateY(-3px)'; }}
      ...
      <div className="psec" id="projects" style={{ background: 'var(--surface)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 24 }}>
          {(projects.length > 0 ? projects : (d.projects || [])).map((p, i) => (
            <div key={p._id || i} style={{ ...card, position: 'relative', overflow: 'hidden', transition: 'all 0.25s' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(0,212,255,0.3)'; e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = '0 20px 40px rgba(0,0,0,0.4)'; }}
              ...
              <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 42, fontWeight: 800, color: 'rgba(0,212,255,0.08)', position: 'absolute', top: 16, right: 24 }}>{String(i + 1).padStart(2, '0')}</div>
              <div style={{ fontSize: 28, marginBottom: 10 }}>{p.icon || '🚀'}</div>
              <div style={{ fontFamily: 'Syne,sans-serif', fontWeight: 700, fontSize: 18, marginBottom: 12 }}>{p.title}</div>
              <p style={{ color: '#9ca3af', fontSize: 14, lineHeight: 1.7, marginBottom: 16 }}>{p.shortDesc || p.desc}</p>
              <div className="tags">{(p.tags || []).map(t => <span key={t} className="tag">{t}</span>)}</div>
              <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
                {p._id && (
                  <Link to={`/projects/${p._id}`} style={{ background: 'var(--accent)', color: '#000', padding: '9px 18px', borderRadius: 8, fontWeight: 700, fontSize: 13, textDecoration: 'none' }}>
                    View Details →
                  </Link>
                )}
                {p.liveUrl && (
                  <a href={p.liveUrl} target="_blank" rel="noreferrer" style={{ background: 'transparent', color: 'var(--accent)', padding: '9px 18px', borderRadius: 8, fontWeight: 600, fontSize: 13, textDecoration: 'none', border: '1px solid rgba(0,212,255,0.3)' }}>
                    🚀 Live Demo
                  </a>
                )}
              </div>
      ...
      <div className="psec" id="contact" style={{ background: 'var(--surface)', textAlign: 'center' }}>
        ...
        <div style={{ maxWidth: 600, margin: '0 auto', ...card, padding: 48 }}>
          <p style={{ color: 'var(--muted)', fontSize: 16, lineHeight: 1.7, marginBottom: 32 }}>Looking for AI & Data Science opportunities or freelance projects. Available for remote full-time and contract work.</p>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 10, flexWrap: 'wrap' }}>
            <a href={`mailto:${d.contact?.email || 'vemunooriramana0602@gmail.com'}`} style={contactBtn}>✉️ Email Me</a>
            <Link to="/resume" style={contactBtn}>📄 Resume</Link>
            <Link to="/freelance" style={contactBtn}>💼 Hire Me</Link>
          ...
          <p style={{ marginTop: 20, fontSize: 13, color: '#4b5563' }}>📍 {d.contact?.location || 'Hyderabad, India'} · {d.contact?.phone || '+91 8499882843'}</p>
        </div>
      </div>
      ...
      <footer style={{ padding: '24px 60px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 16 }}>
          <Link to="/resume" style={{ color: 'var(--muted)', fontSize: 13 }}>📄 Resume</Link>
          <Link to="/freelance" style={{ color: 'var(--muted)', fontSize: 13 }}>💼 Hire Me</Link>
          <Link to="/referrals" style={{ color: 'var(--muted)', fontSize: 13 }}>🔗 Referrals</Link>
        </div>
      </footer>
...
const btnPrimary = { background: 'var(--accent)', color: '#000', padding: '13px 24px', borderRadius: 8, fontWeight: 700, fontSize: 14, textDecoration: 'none' };
const btnSecondary = { background: 'transparent', color: 'var(--text)', padding: '13px 24px', borderRadius: 8, fontWeight: 500, fontSize: 14, textDecoration: 'none', border: '1px solid var(--border)' };
const contactBtn = { display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(0,212,255,0.08)', border: '1px solid rgba(0,212,255,0.2)', color: 'var(--accent)', padding: '12px 20px', borderRadius: 8, fontSize: 14, fontWeight: 500, textDecoration: 'none' };
