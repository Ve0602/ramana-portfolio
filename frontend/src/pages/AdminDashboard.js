...
import { ClientsTab, FreelancersTab, AutoJobsTab } from './FreelanceAdminTabs';

const TABS = ['Dashboard','Brand Settings','Login Page','Home Page','Products','Services','Referrals','Resume','Freelance','Portfolio','Reviews','📧 Notifications','👥 Clients','🧑‍💻 Freelancers','💼 Auto Jobs','Users'];
const ICONS = { Dashboard:'📊', 'Brand Settings':'🎨', 'Login Page':'🔐', 'Home Page':'🏠', Products:'🛍️', Services:'⚙️', Referrals:'🔗', Resume:'📄', Freelance:'💼', Portfolio:'📝', Reviews:'⭐', '📧 Notifications':'📧', '👥 Clients':'👥', '🧑‍💻 Freelancers':'🧑‍💻', '💼 Auto Jobs':'💼', Users:'👥' };
...
function UsersTab({ h, API }) {
  ...
  useEffect(()=>{ axios.get(`${API}/api/admin/users`,{headers:h}).then(r=>setUsers(r.data)); },[]);
  const del = async id => { if(window.confirm('Delete?')){ await axios.delete(`${API}/api/admin/users/${id}`,{headers:h}); setUsers(u=>u.filter(x=>x._id!==id)); }};
  return (
    <div>
      ...
      <div style={card}>
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
          <thead><tr style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
            {['Name','Email','Joined','Last Login',''].map(hd=><th key={hd} style={{textAlign:'left',padding:'8px 12px',color:'rgba(255,255,255,0.4)',fontWeight:600}}>{hd}</th>)}
          </tr></thead>
          <tbody>
            {users.map(u=>(
              <tr key={u._id} style={{borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
                ...
                <td style={{padding:'10px 12px',color:'rgba(255,255,255,0.4)'}}>{new Date(u.createdAt).toLocaleDateString()}</td>
                <td style={{padding:'10px 12px',color:'rgba(255,255,255,0.4)'}}>{u.lastLogin?new Date(u.lastLogin).toLocaleDateString():'Never'}</td>
                <td style={{padding:'10px 12px'}}><Btn onClick={()=>del(u._id)} small danger>Delete</Btn></td>
              </tr>
            ))}
          </tbody>
        </table>
