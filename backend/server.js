...
mongoose.connect(process.env.MONGODB_URI, { serverSelectionTimeoutMS:30000, connectTimeoutMS:30000 })
  .then(async () => {
    ...
    setTimeout(async () => { try { const { syncRSSJobs } = require('./utils/rssScraper'); await syncRSSJobs(); } catch(e){} }, 30000);
    setInterval(async () => { try { const { syncRSSJobs } = require('./utils/rssScraper'); await syncRSSJobs(); } catch(e){} }, 6*60*60*1000);
    console.log('⏰ RSS sync scheduled every 6 hours');
  })
